...
const sessionCookieName = "ovl_session"

// sessionTTL is a provisional default (not specified by the
// architecture doc, which doesn't discuss vessel session lifetime) —
// long enough to cover a work shift without re-login.
...
// vessel binary restart — acceptable for Phase 2 (a restart forcing
// re-login is a minor inconvenience, not a data-loss risk) and not
// specified otherwise by the architecture doc.
